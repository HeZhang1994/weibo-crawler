'''Function (Python3): Crawl Sina Weibo data (text, JPG/GIF images, live photos, and videos) of one Weibo user.
...
from pyCrawler_function import video2gif
...
# 4. Crawl Weibo data (original/retweeted text, JPG/GIF images, live photos, and videos).
...
for cards in cards_list:
    ...
    for card in cards:
        ...
        if card['card_type'] == 9:  # The Weibo post with 'card_type=9' has target data.
            ...
            # 4.4 Crawl live photos.
            # Create a sub-folder for saving live photos.
            path_livephoto = PATH_FOLDER + str(count_card) + '_livephoto/'
            os.mkdir(path_livephoto)

            pv_url_start = 'https://video.weibo.com/media/play?livephoto=//us.sinaimg.cn/'  # Fixed.
            pv_url_end = '.mov&KID=unistore,videomovSrc'  # Fixed.

            # 4.4.1 Crawl the live photos in current Weibo post (original).
            if 'retweeted_status' not in card['mblog']:
                if 'pic_video' in card['mblog']:
                    # E.g., pic_video: "0:000voDMsjx07t57qM583010f0100alDF0k01,1:002HcbwHjx07t57r2K63010f0100aMUk0k01"
                    # '0' and '1' - The serial number of live photos in current Weibo post.
                    # '000voDMsjx07t57qM583010f0100alDF0k01' - The identity code of related live photo.

                    pv_str = card['mblog']['pic_video']  # 'pv_str' <str> - picture and video.
                    pv_list = re.split('[,]', pv_str)

                    for i in range(len(pv_list)):
                        pv_code = re.split('[:]', pv_list[i])[1]
                        pv_url = pv_url_start + pv_code + pv_url_end

                        # Download the i-th live photo as a video.
                        name_pv_v = path_livephoto + str(i + 1) + '.mov'
                        try:
                            print('Download the %s-th live photo as a video.' % (i + 1))
                            urllib.request.urlretrieve(urllib.request.urlopen(pv_url).geturl(), name_pv_v)
                        except:
                            print('*****Error: Failed to download the live photo: %s' % pv_url)
                            # Add download error information to TXT file.
                            with open(PATH_FILE_TXT, 'a', encoding='utf-8') as ff:
                                ff.write('*****Error: Failed to download the live photo：' + pv_code + '\n')

                        # Convert video to GIF image.
                        print('Convert video to GIF image.')
                        video2gif(name_pv_v, path_livephoto, str(i + 1) + '.gif')

            # 4.4.2 Crawl the live photos in current Weibo post (retweeted).
            if 'retweeted_status' in card['mblog']:
                if 'pic_video' in card['mblog']['retweeted_status']:
                    # E.g., pic_video: "0:000voDMsjx07t57qM583010f0100alDF0k01,1:002HcbwHjx07t57r2K63010f0100aMUk0k01"
                    # '0' and '1' - The serial number of live photos in current Weibo post.
                    # '000voDMsjx07t57qM583010f0100alDF0k01' - The identity code of related live photo.

                    pv_str = card['mblog']['retweeted_status']['pic_video']  # 'pv_str' <str> - picture and video.
                    pv_list = re.split('[,]', pv_str)
                    ...
                    for i in range(len(pv_list)):
                        ...
                        # Convert video to GIF image.
                        print('Convert video to GIF image.' % (i + 1))
                        video2gif(name_pv_v, path_livephoto, str(i + 1) + '.gif')
        ...
        # 5.3 Check if the sub-folder for saving live photos is empty. If so, delete it.
        if os.path.exists(path_livephoto):
            if not os.listdir(path_livephoto):
                os.rmdir(path_livephoto)
